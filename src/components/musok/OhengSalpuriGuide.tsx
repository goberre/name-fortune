        ...
        {ORDER.map((o) => {
          ...
          return (
            <div key={o} className="flex flex-col items-center gap-2">
              <div className="flex h-24 w-10 items-end justify-center border border-[var(--mk-border)] bg-[var(--mk-charcoal-light)]">
                <motion.div
                  initial={{ height: 0 }}
              ...
              <span className="text-xs" style={{ color: ob.color }}>
                {o}
              </span>
              <span className="text-[9px] text-[var(--mk-ivory-muted)]">{ob.label}</span>
              {filled && <span className="text-[9px] text-[var(--mk-cinnabar-soft)]">비보</span>}
              {inName && !filled && <span className="text-[9px] text-[var(--mk-ivory-dim)]">名</span>}
            </div>
          );
        })}
      </div>
